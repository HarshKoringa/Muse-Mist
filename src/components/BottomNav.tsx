"use client";

import { useState, useEffect } from "react";
import { usePathname, useRouter } from "next/navigation";
import { Home, ShoppingBag, Sparkles, User } from "lucide-react";
import { createClient } from "@/utils/supabase/client";
import ShopPanel from "@/components/ShopPanel";

type Tab = "home" | "shop" | "routine" | "account";

export default function BottomNav() {
  const pathname = usePathname();
  const router = useRouter();
  const [isShopOpen, setIsShopOpen] = useState(false);
  const [user, setUser] = useState<{ id: string } | null>(null);

  useEffect(() => {
    const supabase = createClient();
    supabase.auth.getUser().then(({ data }) => setUser(data.user));
    const { data: listener } = supabase.auth.onAuthStateChange((_e, session) => {
      setUser(session?.user ?? null);
    });
    return () => listener.subscription.unsubscribe();
  }, []);

  // Close shop panel on route change
  useEffect(() => {
    setIsShopOpen(false);
  }, [pathname]);

  // Scroll lock when shop panel is open
  useEffect(() => {
    if (isShopOpen) {
      document.body.style.overflow = "hidden";
    } else {
      document.body.style.overflow = "";
    }
    return () => {
      document.body.style.overflow = "";
    };
  }, [isShopOpen]);

  const hidden =
    pathname.startsWith("/checkout") || pathname === "/login";

  if (hidden) return null;

  const getActive = (): Tab => {
    if (pathname === "/") return "home";
    if (pathname.startsWith("/routine")) return "routine";
    if (pathname.startsWith("/account") || pathname.startsWith("/orders")) return "account";
    return "home";
  };

  const active = getActive();

  const handleTab = (tab: Tab) => {
    if (tab === "shop") {
      setIsShopOpen((prev) => !prev);
      return;
    }
    setIsShopOpen(false);
    if (tab === "home") {
      router.push("/");
    } else if (tab === "routine") {
      router.push("/routine");
    } else if (tab === "account") {
      if (user) {
        router.push("/account");
      } else {
        router.push("/login");
      }
    }
  };

  const tabs: { key: Tab; label: string; Icon: typeof Home }[] = [
    { key: "home", label: "Home", Icon: Home },
    { key: "shop", label: "Shop", Icon: ShoppingBag },
    { key: "routine", label: "Routine", Icon: Sparkles },
    { key: "account", label: "Account", Icon: User },
  ];

  return (
    <>
      <ShopPanel isOpen={isShopOpen} onClose={() => setIsShopOpen(false)} />

      <nav
        className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-[#F3F4F6]"
        style={{ paddingBottom: "env(safe-area-inset-bottom)" }}
      >
        <div className="flex items-stretch h-16">
          {tabs.map(({ key, label, Icon }) => {
            const isActive = key === "shop" ? isShopOpen : active === key;
            return (
              <button
                key={key}
                onClick={() => handleTab(key)}
                style={{ minHeight: "44px", minWidth: "44px" }}
                className="flex-1 flex flex-col items-center justify-center gap-0.5 cursor-pointer"
              >
                <Icon
                  size={22}
                  strokeWidth={isActive ? 2.2 : 1.6}
                  color={isActive ? "#1A237E" : "#9CA3AF"}
                />
                <span
                  style={{
                    fontFamily: "var(--font-body)",
                    fontSize: "10px",
                    fontWeight: isActive ? 600 : 400,
                    color: isActive ? "#1A237E" : "#9CA3AF",
                  }}
                >
                  {label}
                </span>
              </button>
            );
          })}
        </div>
      </nav>
    </>
  );
}
